...
Pout<< "** Merging " << returnReduce(pointToMaster.size(), sumOp<label>())
    << " points" << endl;
forAllConstIter(Map<label>, pointToMaster, iter)
{
    Pout<< "    merging " << mesh_.points()[iter.key()]
        << " into master:" << mesh_.points()[iter()] << endl;
}
