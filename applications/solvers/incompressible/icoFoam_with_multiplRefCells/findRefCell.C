...
)
{
    ...
    const regionSplit& regions = regionSplit::New(mesh);


    // Check per region if there is any fixed boundary condition.

    boolList regionNeedReference(regions.nRegions(), true);
    if (!forceReference)
    {
        const volScalarField::Boundary& bfld = fieldRef.boundaryField();
        forAll(bfld, patchI)
        {
            if (bfld[patchI].fixesValue())
            {
                const labelUList& fc = bfld[patchI].patch().faceCells();
                forAll(fc, i)
                {
                    regionNeedReference[regions[fc[i]]] = false;
                }
            }
        }
        Pstream::listCombineGather(regionNeedReference, andEqOp<bool>());
        Pstream::listCombineScatter(regionNeedReference);
    }


    refCells.setSize(regions.nRegions());
    refCells = -1;
    refValues.setSize(regions.nRegions());
    refValues = pTraits<scalar>::max;

    bool needReference = (findIndex(regionNeedReference, true) != -1);

    if (needReference || forceReference)
    {
        if (dict.found(refCellName))
        {
            if (Pstream::master())
            {
                labelList dictRefCells(dict.lookup(refCellName));

                forAll(regionNeedReference, regionI)
                {
                    if (forceReference || regionNeedReference[regionI])
                    {
                        refCells[regionI] = dictRefCells[regionI];
                    }
                }
            }
        }
        ...
        {
            pointField dictRefPoints(dict.lookup(refPointName));

            // Try fast approximate search avoiding octree construction
            forAll(dictRefPoints, i)
            {
                const point& refPt = dictRefPoints[i];

                label refCelli = mesh.findCell(refPt, polyMesh::FACE_PLANES);
                ...
                (
                    forceReference
                 || (refCelli >= 0 && regionNeedReference[regions[refCelli]])
                )
                {
                    if (sumHasRef != 1)
                    {
                ...
                if (refCelli >= 0)
                {
                    refCells[regions[refCelli]] = refCelli;
                }
            }
        }
        else
        ...
        dict.lookup(refValueName) >> refValues;
    }
}
