...
DebugVar(srcField.size());
DebugVar(result.size());
DebugVar(singleMeshProc_);
...
Pout<< "cell:" << celli
    << " depends on slots:" << srcAddress
    << " with weights:" << srcWeight << endl;
...
Pout<< "cell:" << celli
    << " depends on local cells:" << srcAddress
    << " with weights:" << srcWeight << endl;
