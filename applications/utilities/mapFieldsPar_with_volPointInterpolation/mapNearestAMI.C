...
)
{
    ...
    List<DynamicList<scalar>> tgtMagSf(this->tgtPatch_.size());
    ...
    do
    {
        ...
        tgtMagSf[tgtFacei].append(this->tgtMagSf_[tgtFacei]);
    ...
    forAll(tgtAddr, targetFacei)
    {
        if (tgtAddr[targetFacei].size() > 1)
        {
            ...
            // Adapt area accordingly
            DynamicList<scalar>& weights = tgtMagSf[targetFacei];
            weights.clear();
            weights.append(this->tgtMagSf_[targetFacei]);
        }
    }
...
DebugVar(srcAddr);
DebugVar(this->srcMagSf_);
DebugVar(tgtAddr);
DebugVar(this->tgtMagSf_);
DebugVar(tgtMagSf);
    ...
    forAll(tgtAddr, i)
    {
        ...
        tgtWeights[i].transfer(tgtMagSf[i]);
    }
}
