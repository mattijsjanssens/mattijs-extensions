...
//- Optional coupled-cell sorting
enum class coupledCellSorting : char
{
    FALSE,
    TRUE,
    FIRST
...
//- Enumerated names
static const Enum<coupledCellSorting> coupledCellSortingNames
({
    {coupledCellSorting::FALSE, "false" },
    {coupledCellSorting::TRUE, "true" },
    {coupledCellSorting::FIRST, "first" }
});
...
int main(int argc, char *argv[])
{
    ...
    for (fvMesh& mesh : meshes)
    {
        ...
        coupledCellSorting sortCoupledFaceCells = coupledCellSorting::FALSE;
        ...
        if (readDict)
        {
            ...
            sortCoupledFaceCells = coupledCellSortingNames.getOrDefault
            (
                "sortCoupledFaceCells",
                renumberDict,
                coupledCellSorting::FALSE
            );

            if (sortCoupledFaceCells == coupledCellSorting::TRUE)
            {
                Info<< "Sorting cells on coupled boundaries to be last." << nl
                    << endl;
            }
            else if (sortCoupledFaceCells == coupledCellSorting::FIRST)
            {
                Info<< "Sorting cells on coupled boundaries to be first." << nl
                    << endl;
            }
        ...
        else
        {
            ...
            if (sortCoupledFaceCells != coupledCellSorting::FALSE)
            {
