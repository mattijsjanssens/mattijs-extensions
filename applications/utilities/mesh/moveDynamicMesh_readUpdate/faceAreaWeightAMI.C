...
) const
{
    ...
    const scalar srcMag = this->srcMagSf_[srcFacei];
    const scalar tgtMag = tgt.mag(tgtPoints);
    if ((srcMag < ROOTVSMALL) || (tgtMag < ROOTVSMALL))
    {
        return area;
    }

    const vector& srcN = this->srcPatch_.faceNormals()[srcFacei];
    const vector& tgtN = this->tgtPatch_.faceNormals()[tgtFacei];

    if ((srcN&tgtN) > -0.9)
    {
Pout<< "srcN:" << srcN << " tgtN:" << tgtN
    << " cos:" << (srcN&tgtN) << endl;
