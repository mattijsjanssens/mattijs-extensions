...
{
    fileName fName("TargetPatch_" + Foam::name(Pstream::myProcNo()) + ".obj");
    Pout<< "Dumping to " << fName << endl;
    OBJstream str(fName);
    str.write(newTgtFaces, newTgtPoints, false);
}
...
//             )
//         );
//         AMIMethod<SourcePatch, TargetPatch>& AMI = AMIPtr();

        faceAreaWeightAMI<SourcePatch, TargetPatch> AMI
        (
            srcPatch,
            newTgtPatch,
            srcMagSf_,
...
Pout<< "** type:" << AMI.type() << endl;
        ...
        if (debug)
        {
            ...
            {
                const point& fc = srcPatch.faceCentres()[srcI];
                const vector area = srcPatch[srcI].normal(srcPatch.points());
                scalar srcMagSf = srcPatch[srcI].mag(srcPatch.points());

                const labelList& src = srcAddress_[srcI];
                const scalarList& wghts = srcWeights_[srcI];

                scalar sumW = sum(wghts);
                scalar f = sumW/srcMagSf;
                if (mag(f-1.0) > 1e-3)
                {
                    Pout<< "SrcFace:" << srcI
                        << " centre:" << fc
                        << " area:" << mag(area)
                        << " normal:" << area/mag(area)
                        << " overlapped by:" << f << endl;
                    forAll(src, i)
                    {
                        label tgtI = src[i];
                        const point& tgtFc = newTgtPatch.faceCentres()[tgtI];
                        const vector tgtArea =
                            newTgtPatch[tgtI].normal(newTgtPatch.points());

                        Pout<< "    tgtFace:" << tgtI
                            << " centre:" << tgtFc
                            << " area:" << mag(tgtArea)
                            << " normal:" << tgtArea/mag(tgtArea)
                            << " overlap:" << wghts[i]
                            << endl;
                    }
                }
            ...
            {
                const point& fc = newTgtPatch.faceCentres()[tgtI];
                const vector area = newTgtPatch[tgtI].normal(newTgtPatch.points());
                scalar tgtMagSf = newTgtPatch[tgtI].mag(newTgtPatch.points());

                const labelList& tgt = tgtAddress_[tgtI];
                const scalarList& wghts = tgtWeights_[tgtI];

                scalar sumW = sum(wghts);
                scalar f = sumW/tgtMagSf;
                if (mag(f-1.0) > 1e-3)
                {
                    Pout<< "tgtFace:" << tgtI
                        << " centre:" << fc
                    ...
                    {
                        label srcI = tgt[i];
                        const point& srcFc = srcPatch.faceCentres()[srcI];
                        const vector srcArea =
                            srcPatch[srcI].normal(srcPatch.points());

                        Pout<< "    srcFace:" << srcI
                            << " centre:" << srcFc
                            << " area:" << mag(srcArea)
                            << " normal:" << srcArea/mag(srcArea)
                            << " overlap:" << wghts[i]
                            << endl;
                    }
                }
        ...
        forAll(srcAddress_, srcI)
        {
            ...
            labelHashSet set(2*src.size());
            forAll(src, i)
            {
                if (!set.insert(src[i]))
                {
                    Pout<< "BEFORE Source Duplicate at:" << srcI
                        << " addresses:" << src
                        << " weights:" << wghts << endl;
                }
            }
        ...
        forAll(tgtAddress_, tgtI)
        {
            ...
            const scalarList& wghts = tgtWeights_[tgtI];
            // Check for duplicates
            labelHashSet set(2*tgt.size());
            forAll(tgt, i)
            {
                if (!set.insert(tgt[i]))
                {
                    Pout<< "BEFORE Target Duplicate at:" << tgtI
                        << " addresses:" << tgt
                        << " weights:" << wghts << endl;
                }
            }
