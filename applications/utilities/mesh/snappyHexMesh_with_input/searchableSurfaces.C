...
namespace Foam
{
    ...
    <
        Foam::searchableSurfaces::namingType,
        3
    >::names[] =
    {
        "surfaceAndRegion",
...
const Foam::NamedEnum<Foam::searchableSurfaces::namingType, 3>
Foam::searchableSurfaces::namingTypeNames;
    ...
    allSurfaces_(identity(topDict.size()))
{
    ...
    // From global region name to original surface. Used to check for
    // naming conflicts
    HashTable<word> regionToSurface(2*topDict.size());
    ...
    forAllConstIter(dictionary, topDict, iter)
    {
        ...
        // Construct default region names according to naming rule.
        const wordList& localNames = s.regions();
        ...
        switch (namingRule)
        {
            ...
            case SINGLE_REGION:
            {
                ...
                else
                {
                    ...
                    {
                        rNames[regionI] =
                            names_[surfI] + '_' + localNames[regionI];
                    }
                }
            }
            break;
        ...
        {
            if (!regionToSurface.insert(rNames[regionI], key))
            {
                IOWarningInFunction(topDict)
                    << "Region name " << rNames[regionI]
                    << " conflicts with region name for surface "
                    << regionToSurface[rNames[regionI]]
                    << endl;
            }
        }
