...
int main(int argc, char *argv[])
{
    ...
    forAllConstIter(dictionary, dict, iter)
    {
        ...
        word extractionMethod;
        fileNameList surfFileNames;
        if (surfaceDict.readIfPresent("surfaces", surfFileNames))
        {
            extractionMethod = iter().keyword();
        }
        else
        {
            surfFileNames = fileNameList(1, iter().keyword());
            if
            (
               !surfaceDict.readIfPresent("extractionMethod", extractionMethod)
            )
            {
                continue;
            }
        ...
        forAll(surfFileNames, i)
        {
            const fileName surfFileName = surfFileNames[i];
            ...
            else
            {
                FatalErrorInFunction
                    ...
                    << " (to construct new set from angle)"
                    << exit(FatalError);
            }
            ...
            if (surfaceDict.isDict("subsetFeatures"))
            {
                ...
                const Switch nonManifoldEdges =
                    subsetDict.lookupOrDefault<Switch>
                    (
                        "nonManifoldEdges",
                        "yes"
                    );
            ...
            if (surfaceDict.isDict("addFeatures"))
            {
                const word addFeName =
                    surfaceDict.subDict("addFeatures")["name"];
        ...
        // );
            ...
            if (closeness)
            {
                ...
                Info<< "externalToleranceCosAngle: "
                ...
                forAll(allHitInfo, fI)
                {
                    ...
                    else
                    {
                        ...
                        forAll(hitInfo, hI)
                        {
                            // Find the hit on the triangle that launched the
                            // ray
                        ...
                        {
                            // There are no external hits, the last but one hit
                            // is the closest internal hit
            ...
            if (curvature)
            {
                ...
                triadField pointCoordSys =
                    calcVertexCoordSys(surf, pointNormals);
            ...
            if (featureProximity)
            {
                ...
                forAll(surf, fI)
                {
                    ...
                    feMesh.allNearestFeaturePoints
                    (
                        triCentre,
                        radiusSqr,
                        hitList
