...
) const
{
    ...
    forAll(pointCells, pointi)
    {
        if (!isPatchPoint_[pointi])
        {
            ...
            Pout<< "internal point:" << vf.mesh().points()[pointi]
                << " has value-from-cells:" << pf[pointi] << endl;
        }
    }
}
...
) const
{
    ...
    forAll(boundaryVals, bFacei)
    {
        label facei = vf.mesh().nInternalFaces()+bFacei;
        Pout<< "fc:" << vf.mesh().faceCentres()[facei]
            << " fc2:" << boundary[bFacei].centre(boundary.points())
            << " value:" << boundaryVals[bFacei]
            << endl;
    }
    ...
    forAll(boundary.meshPoints(), i)
    {
        ...
        if (isPatchPoint_[pointi])
        {
            ...
            bool dump =
                (mag(vf.mesh().points()[pointi]-point(1, 0.5, 0.5)) < 1e-6);
            ...
            forAll(pFaces, j)
            {
                if (boundaryIsPatchFace_[pFaces[j]])
                {
                    if (dump)
                    {
                        label bFacei = pFaces[j];
                        Pout<< "patch point:" << vf.mesh().points()[pointi]
                            << " gets boundary contribution:"
                            << " from face:"
                            << boundary[bFacei].centre(boundary.points())
                            << " val:" << boundaryVals[bFacei]
                            << " weight:" << pWeights[j]
                            << endl;
                    }
            ...
            Pout<< "patch point:" << vf.mesh().points()[pointi]
                << " has value-from-uncoupled-patches:" << val << endl;
        }
    }
...
forAll(vf.mesh().points(), pointi)
{
    Pout<< "after interpolateBoundaryField point:" << vf.mesh().points()[pointi]
        << " has value:" << pf[pointi] << endl;
}
    // Apply constraints
...
forAll(vf.mesh().points(), pointi)
{
    Pout<< "after constrains point:" << vf.mesh().points()[pointi]
        << " has value:" << pf[pointi] << endl;
}
}
