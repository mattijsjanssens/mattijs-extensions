...
DebugVar(myData.size());
DebugVar(slaves.size());
DebugVar(transformedSlaves.size());
Pout<< "start of distributed myData:" << nl;
forAll(myData, i)
{
    const labelPair& n = myData[i];
    label procI = globalIndexAndTransform::processor(n);
    label pointI = globalIndexAndTransform::index(n);
    label transformI = globalIndexAndTransform::transformIndex(n);

    Pout<< indent << "point:" << pointI << nl;
    Pout<< indent << "at:" << coupledPatch().localPoints()[pointI] << nl;
    Pout<< indent << "procI:" << procI << nl;
    Pout<< indent << "transformI:" << transformI << nl << nl;
}
Pout<< "end of distributed myData" << nl;
...
Pout<< "** on master:" << endl;
forAll(allPointConnectivity, i)
{
    const labelPairList& conn = allPointConnectivity[i];

    Pout<< "Connected:" << nl << incrIndent;
    forAll(conn, connI)
    {
        const labelPair& lp = conn[connI];
        label pointI = globalIndexAndTransform::index(lp);
        label procI = globalIndexAndTransform::processor(lp);
        label transformI = globalIndexAndTransform::transformIndex(lp);
    ...
    Pout<< decrIndent << endl;
}
Pout<< "** end of master:" << endl;
...
DebugVar(allPointConnectivity);
forAll(allPointConnectivity, pointI)
{
    const labelPairList& conn = allPointConnectivity[pointI];

    Pout<< "point:" << pointI
        << " at:" << coupledPatch().localPoints()[pointI]
        <<" connected to:" << nl << incrIndent;
    forAll(conn, connI)
    {
        const labelPair& lp = conn[connI];
        label ptI = globalIndexAndTransform::index(lp);
        label procI = globalIndexAndTransform::processor(lp);
        label transformI = globalIndexAndTransform::transformIndex(lp);

        Pout<< indent << "point:" << ptI << nl;
        Pout<< indent << "at:" << coupledPatch().localPoints()[ptI] << nl;
        Pout<< indent << "procI:" << procI << nl;
        Pout<< indent << "transformI:" << transformI << nl << nl;
    }
    Pout<< decrIndent << endl;
...
DebugVar(globalPointEdges);
DebugVar(globalPointPoints);
forAll(globalPointEdges, pointI)
{
    Pout<< "point:" << pointI
        << " at:" << coupledPatch().localPoints()[pointI]
        << nl;
    const labelPairList& conn = globalPointPoints[pointI];
    const labelList& edgeIDs = globalPointEdges[pointI];
    ...
    forAll(conn, connI)
    {
        ...
        label edgeI = edgeIDs[connI];
        const edge& e = coupledPatch().edges()[edgeI];

        Pout<< indent << "edge:" << edgeI << nl
            << indent << "start:" << e.start()
            << " at:" << coupledPatch().localPoints()[e.start()] << nl
            << indent << "end  :" << e.end()
            << " at:" << coupledPatch().localPoints()[e.end()] << nl << nl;
    }
    Pout<< decrIndent << endl;
}
...
{
    label coupledEdgeI = pEdges0[i];
    const edge& cppEdge = edges[coupledEdgeI];

Pout<< "for edge:" << edgeI
    << " verts:" << e
    << " at:" << coupledPatch().localPoints()[e[0]]
    << coupledPatch().localPoints()[e[1]]
    << " found coupled edge:" << coupledEdgeI
    << " verts:" << cppEdge
    << " at:" << coupledPatch().localPoints()[cppEdge[0]]
    << coupledPatch().localPoints()[cppEdge[1]]
    << " to point:" << pPoints0[i]
    << " to point:" << pPoints1[j]
    << nl << endl;
}
