    ...
    forAll(srcNbrFaces, i)
    {
        ...
        if (mapFlag[faceS] && seedFaces[faceS] == -1)
        {
            forAll(visitedFaces, j)
            {
                ...
                scalar area = interArea(faceS, faceT).first();
...
Foam::faceAreaWeightAMI<SourcePatch, TargetPatch>::interArea
(
    const label srcFacei,
...
) const
{
    ...
    if (this->srcMagSf_[srcFacei] < rootVSmall)
    {
