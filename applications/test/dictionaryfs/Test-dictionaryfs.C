...
#define BB_DATA (reinterpret_cast<struct bb_state *>(fuse_get_context()->private_data))
...
{
    strcpy(fpath, BB_DATA->rootdir.c_str());
    strncat(fpath, path, PATH_MAX); // ridiculously long paths will
				    // break here

    log_msg("    bb_fullpath:  rootdir = \"%s\", path = \"%s\", fpath = \"%s\"\n",
	    BB_DATA->rootdir.c_str(), path, fpath);
}
	       ...
	       struct fuse_file_info *fi)
{
    ...
    dp = reinterpret_cast<DIR *>(reinterpret_cast<uintptr_t>(fi->fh));
...
int bb_releasedir(const char *path, struct fuse_file_info *fi)
{
    ...
    closedir(reinterpret_cast<DIR *>(reinterpret_cast<uintptr_t>(fi->fh)));
...
{
    argList::validArgs.append("rootDir");
