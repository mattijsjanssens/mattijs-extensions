...
) const
{
    ...
    DynamicList<label> seedFaces(mesh().nFaces()-mesh().nInternalFaces());
    ...
    )
    {
        if (!isConnection[facei])
        {
            ...
            seedData.append(minData(globalFaces.toGlobal(facei)));
        }
    }

    if (debug)
    {
        Pout<< "Seeded " << seedFaces.size() << " boundary faces." << endl;
    }
    ...
    if (blockedFace.size())
    {
        PackedBoolList isCellOnBlockedFace(mesh().nCells());

        forAll(blockedFace, facei)
        {
            if (blockedFace[facei])
            {
                isCellOnBlockedFace[mesh().faceOwner()[facei]] = true;
                if (mesh().isInternalFace(facei))
                {
                    isCellOnBlockedFace[mesh().faceNeighbour()[facei]] = true;
                }
            }
        }

        forAll(isConnection, facei)
        {
            if (!blockedFace[facei] && isConnection[facei])
            {
                label own = mesh().faceOwner()[facei];
                if (mesh().isInternalFace(facei))
                {
                    label nei = mesh().faceNeighbour()[facei];
                    if (isCellOnBlockedFace[own] != isCellOnBlockedFace[nei])
                    {
                        seedFaces.append(facei);
                        seedData.append(minData(globalFaces.toGlobal(facei)));
                    }
                }
                else if (isCellOnBlockedFace[own])
                {
                    // Unblocked coupled boundary. Need to seed at least
                    // one of the faces in the region.
                    seedFaces.append(facei);
                    seedData.append(minData(globalFaces.toGlobal(facei)));
                }
            }
        ...
        if (debug)
        {
            Pout<< "Seeded " << seedFaces.size()
                << " boundary and internal faces." << endl;
        }
    }
