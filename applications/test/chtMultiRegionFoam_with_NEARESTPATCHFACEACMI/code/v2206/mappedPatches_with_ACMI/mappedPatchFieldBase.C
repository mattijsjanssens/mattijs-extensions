...
template<class Type>
Type Foam::mappedPatchFieldBase2<Type>::getAverage
(
...
template<class Type>
template<class T>
void Foam::mappedPatchFieldBase2<Type>::storeField
(
...
) const
{
    ...
    forAll(procToMap, ranki)
    {
        ...
        if (map.size())
        {
            ...
            mappedPatchBase2::storeField(subObr, fieldName, subFld);
        }
    }
}
...
template<class Type>
template<class T>
bool Foam::mappedPatchFieldBase2<Type>::retrieveField
(
    ...
    forAll(procToMap, ranki)
    {
        ...
        if (map.size())
        {
            ...
            else if (allowUnset)
            {
                ...
                mappedPatchBase2::storeField(subObr, fieldName, dummyFld);
...
template<class Type>
template<class T>
void Foam::mappedPatchFieldBase2<Type>::initRetrieveField
(
    const objectRegistry& obr,
    const word& region,
    const word& patch,
...
) const
{
    ...
    for (label domain = 0; domain < nProcs; domain++)
    {
        ...
        if (constructMap.size())
        {
            ...
            mappedPatchBase2::storeField(subObr, fieldName, receiveFld);
        }
    }
}
...
template<class Type>
template<class T>
bool Foam::mappedPatchFieldBase2<Type>::storeAndRetrieveField
(
    const word& fieldName,
    const label myComm,
...
template<class Type>
Foam::mappedPatchFieldBase2<Type>::mappedPatchFieldBase2
(
    const mappedPatchBase2& mapper,
    ...
    interpolationScheme_(interpolationCell<Type>::typeName)
{
    ...
    (
     ...
     && (
            mapper_.mode() != mappedPatchBase2::NEARESTPATCHFACE
         && mapper_.mode() != mappedPatchBase2::NEARESTPATCHFACEAMI
         && mapper_.mode() != mappedPatchBase2::NEARESTPATCHFACEACMI
        )
    )
    {
        FatalErrorInFunction
            ...
            << "sampleModes "
            <<  mappedPatchBase2::sampleModeNames_
                [
                    mappedPatchBase2::NEARESTPATCHFACE
                ]
            << " and "
            <<  mappedPatchBase2::sampleModeNames_
                [
                    mappedPatchBase2::NEARESTPATCHFACEAMI
                ]
            << " and "
            <<  mappedPatchBase2::sampleModeNames_
                [
                    mappedPatchBase2::NEARESTPATCHFACEACMI
                ]
            << exit(FatalError);
    }

    if (mapper_.mode() == mappedPatchBase2::NEARESTCELL)
    {
...
:
    mappedPatchFieldBase2<Type>::mappedPatchFieldBase2(mapper, patchField, dict)
{
    if (mapper_.sampleDatabase())
    {
        if (mapper_.mode() == mappedPatchBase2::NEARESTPATCHFACE)
        {
...
Foam::mappedPatchFieldBase2<Type>::mappedPatchFieldBase2
(
    const mappedPatchFieldBase2<Type>& mapper
)
:
...
(
    const mappedPatchBase2& mapper,
    const fvPatchField<Type>& patchField,
    const mappedPatchFieldBase2<Type>& base
)
:
    mapper_(mapper),
    patchField_(patchField),
...
Foam::mappedPatchFieldBase2<Type>::sampleField(const word& fieldName) const
{
...
Foam::mappedPatchFieldBase2<Type>::sampleField() const
{
...
template<class Type>
template<class T>
void Foam::mappedPatchFieldBase2<Type>::distribute
(
    const word& fieldName,
    Field<T>& fld
) const
{
    if (mapper_.mode() == mappedPatchBase2::NEARESTPATCHFACEACMI)
    {
        Pout<< "** mappedPatchFieldBase2<Type>::distribute" << endl;
...
Foam::mappedPatchFieldBase2<Type>::mappedField
(
...
) const
{
    ...
    switch (mapper_.mode())
    {
        case mappedPatchBase2::NEARESTCELL:
        {
        ...
        case mappedPatchBase2::NEARESTPATCHFACE:
        case mappedPatchBase2::NEARESTPATCHFACEAMI:
        case mappedPatchBase2::NEARESTPATCHFACEACMI:
        {
            ...
            if (mapper_.mode() == mappedPatchBase2::NEARESTPATCHFACEACMI)
            {
                Pout<< "** mappedPatchFieldBase2<Type>::mappedField" << endl;
                const AMIPatchToPatchInterpolation& AMI = mapper_.AMI();
                DebugVar(AMI.srcWeightsSum());
                DebugVar(AMI.tgtWeightsSum());
            }
        ...
        case mappedPatchBase2::NEARESTFACE:
        {
...
//template<class Type>
//Foam::tmp<Foam::Field<Type>>
//Foam::mappedPatchFieldBase2<Type>::mappedField() const
//{
...
template<class Type>
Foam::tmp<Foam::Field<Type>>
Foam::mappedPatchFieldBase2<Type>::mappedInternalField() const
{
...
Foam::mappedPatchFieldBase2<Type>::mappedWeightField() const
{
    // Swap to obtain full local values of neighbour internal field
...
template<class Type>
void Foam::mappedPatchFieldBase2<Type>::mappedWeightField
(
    const word& fieldName,
...
template<class Type>
const Foam::mappedPatchBase2& Foam::mappedPatchFieldBase2<Type>::mapper
(
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF
)
{
    if (!isA<mappedPatchBase2>(p.patch()))
    {
        FatalErrorInFunction
    ...
    return refCast<const mappedPatchBase2>(p.patch());
}
...
template<class Type>
void Foam::mappedPatchFieldBase2<Type>::write(Ostream& os) const
{
