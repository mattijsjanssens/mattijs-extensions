...
:
    ...
    mappedPatchFieldBase2<scalar>
    (
        mappedPatchFieldBase2<scalar>::mapper(p, iF),
        *this
    ),
...
turbulentTemperatureRadCoupledMixedFvPatchScalarField2
(
    const turbulentTemperatureRadCoupledMixedFvPatchScalarField2& psf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
:
    ...
    nonOverlapPatchFieldPtr_
    (
        (mapper().mode() == mappedPatchBase2::NEARESTPATCHFACEACMI)
      ? fvPatchField<scalar>::New
        (
            p,
            iF,
...
{
    if (!isA<mappedPatchBase2>(this->patch().patch()))
    {
        FatalErrorInFunction
            << "' not type '" << mappedPatchBase2::typeName << "'"
            << "\n    for patch " << p.name()
...
:
    ...
    mappedPatchFieldBase2<scalar>
    (
        mappedPatchFieldBase2<scalar>::mapper(patch(), iF),
        *this,
        psf
    ),
    TnbrName_(psf.TnbrName_),
...
:
    ...
    mappedPatchFieldBase2<scalar>
    (
        mappedPatchFieldBase2<scalar>::mapper(patch(), psf.internalField()),
        *this,
        psf
    ),
    TnbrName_(psf.TnbrName_),
...
turbulentTemperatureRadCoupledMixedFvPatchScalarField2::nonOverlapPatchField()
const
{
...
void turbulentTemperatureRadCoupledMixedFvPatchScalarField2::autoMap
(
    const fvPatchFieldMapper& mapper
)
{
    ...
    //mappedPatchFieldBase2<scalar>::autoMap(mapper);
    if (thicknessLayer_)
    {
...
void turbulentTemperatureRadCoupledMixedFvPatchScalarField2::rmap
(
...
)
{
    ...
    const turbulentTemperatureRadCoupledMixedFvPatchScalarField2& tiptf =
        refCast
        <
    ...
    //mappedPatchFieldBase2<scalar>::rmap(ptf, addr);
    if (thicknessLayer_)
    {
...
turbulentTemperatureRadCoupledMixedFvPatchScalarField2::kappa
(
...
void turbulentTemperatureRadCoupledMixedFvPatchScalarField2::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    // Get the coupling information from the mappedPatchBase2
    const label patchi = patch().index();
    const mappedPatchBase2& mpp =
        mappedPatchFieldBase2<scalar>::mapper
        (
            patch(),
            this->internalField()
        );
    ...
    if (mapper().mode() == mappedPatchBase2::NEARESTPATCHFACEACMI)
    {
...
void turbulentTemperatureRadCoupledMixedFvPatchScalarField2::manipulateMatrix
(
    fvMatrix<scalar>& m,
    const label iMatrix,
...
tmp<Field<scalar>> turbulentTemperatureRadCoupledMixedFvPatchScalarField2::coeffs
(
...
tmp<scalarField>
turbulentTemperatureRadCoupledMixedFvPatchScalarField2::alphaSfDelta() const
{
    return (alpha(*this)*patch().deltaCoeffs()*patch().magSf());
}


tmp<scalarField> turbulentTemperatureRadCoupledMixedFvPatchScalarField2::
beta() const
{
    const mappedPatchBase2& mpp =
        refCast<const mappedPatchBase2>(patch().patch());
...
void turbulentTemperatureRadCoupledMixedFvPatchScalarField2::write
(
    Ostream& os
) const
{
    ...
    mappedPatchFieldBase2<scalar>::write(os);
}
