...
bool Foam::ISstream::keepComments = false;
...
char Foam::ISstream::nextValid(std::string& comment)
{
    char c = 0;
    comment = string::null;
...
Foam::Istream& Foam::ISstream::read(token& t)
{
    ...
    switch (c)
    {
        ...
        case token::DIVIDE :
        {
            ...
            {
                //std::cout<< "** stealing comment:" << comment << std::endl;
                //t = std::move(comment); // Move contents to token
                //t.setType(token::tokenType::VERBATIM);
                //t = token(comment, t.lineNumber());

                // Convert into #comment
                t = word("#comment");
                t.setType(token::tokenType::DIRECTIVE);

                //std::cout<< "** Returning directive " << t.info()
                //    << std::endl;
            }
            return *this;
        }
