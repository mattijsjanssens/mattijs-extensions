...
) const
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                faceList masterLocalFaces;
                pointField masterLocalPoints;

                UIPstream fromNeighbour(neighbProcNo(), pBufs);
                fromNeighbour >> masterCtrs >> masterNormals
                              >> masterAnchors >> masterFacePointAverages
                              >> masterLocalFaces >> masterLocalPoints;

                primitiveFacePatch masterPp
                (
                    masterLocalFaces,
                    masterLocalPoints
                );

                {
                    Pout<< "master:"
                        << " faces:" << masterPp.size()
                        << " points:" << masterPp.nPoints()
                        << " edges:" << masterPp.nEdges() << endl;
                    Pout<< "local:"
                        << " faces:" << pp.size()
                        << " points:" << pp.nPoints()
                        << " edges:" << pp.nEdges() << endl;
                }


                if
                (
                    masterPp.size() != pp.size()
                 || masterPp.nPoints() != pp.nPoints()
                 || masterPp.nEdges() != pp.nEdges()
                )
                {
                    {
                        Pout<< "master:"
                    ...
                    // Dump faces
                    {
                        ...
                        (
                            boundaryMesh().mesh().time().path()
                           /name() + "_masterfaces.obj"
                        );
                        Pout<< "processorPolyPatch::order :"
                            << " Writing master faces to OBJ file "
                            << str.name() << endl;
                        writeOBJ(str, masterPp, masterPp.points());
                    }
                    {
                        fileName str
                        (
                        ...
                        Pout<< "processorPolyPatch::order :"
                            << " Writing local faces to OBJ file "
                            << str.name() << endl;
                        writeOBJ(str, pp, pp.points());
                    }
                    FatalErrorInFunction << "problem on patch:" << name()
                        << exit(FatalError);
                }
            }
