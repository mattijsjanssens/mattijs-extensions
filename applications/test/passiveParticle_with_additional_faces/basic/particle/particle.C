...
(
    ...
    const labelUList& cellTriangles
)
{
    scalar f = 1;

    const label initTetTriI = onFace() ? 0 : -1;

    label tetTriI = initTetTriI;
    ...
    while (true)
    {
        ...
        //const label oldTetTriI(tetTriI);
        //scalar maxFraction = -GREAT;
        //label maxTetTriI = -1;
        //if (immersedBoundaryFaceCells_[celli_].size())
        //{
        //    const labelList& addFaces = immersedBoundaryFaceCells_[celli_];
        //    for (const label facei : addFaces)
        //    {
        //        label tetTriI = initTetTriI;
        //        const scalar s =
        //            trackToTri(facei, f*displacement, f*fraction, tetTriI);
        //        if (s < maxFraction)
        //        {
        //            maxFraction = s;
        //            maxTetTriI = tetTriI;
...
(
    ...
    //- Track
    const vector& displacement, // total displacement
    const scalar fraction,      // current fraction
    const scalar stepFraction,  // starting fraction

    //- Result from tracking
    label& tetTriI,             // new tetIndex
    barycentric& newCoordinates,// new coordinates
    scalar& newStepFraction     // new fraction
)
{
...
//MEJ
    ...
    barycentric Tx1(f*displacement & T);
    ...
    barycentric yH = coordinates + muH*Tx1;
    ...
    newStepFraction = stepFraction + fraction*muH*detA;
...
//MEJ
        point centre, base, vertex1, vertex2;
        stationaryTetGeometry(centre, base, vertex1, vertex2);

        barycentric newCoordinates;
        scalar newStepFraction;
        const scalar s = trackToStationaryTri
        (
            debug,
            ...
            stepFraction(), // starting fraction

            //- Result from tracking
            tetTriI,        // new tetIndex
        ...
        {
            const triFace& tri = immFaces[triI];
            const point& base = immPoints[tri[0]];
            ...
            Pout<< "Immersed tet:"
                ...
                << " origposition:" << (A&coordinates)
                << endl;

            barycentric immCoordinates;
            scalar immStepFraction;
            label immTetTriI = tetTriI;

            const scalar immFraction = trackToStationaryTri
            (
                debug,
                ...
                //- Result from tracking
                immTetTriI,     // new tetIndex
                immCoordinates, // new coordinates
