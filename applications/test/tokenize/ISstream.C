...
char Foam::ISstream::nextValid(label& column)
{
    char c = 0;
    ...
    while (true)
    {
        ...
        if (c == '/')
        {
            ...
            if (c == '/')
            {
                if (labelRange::debug)
                {
                    //return '&'; // Special character to denote C++ comment
                ...
                {
                    // C++ style single-line comment - skip through past
                    // end-of-line
                    while (get(c) && c != '\n')
                    {}
                }
...
Foam::Istream& Foam::ISstream::read(token& t)
{
    ...
    switch (c)
    {
        case '&' :
        {
            ...
            // buf[nChar] = '\0';
            // 
            // std::cout<< "Read comment:" << buf << std::endl;
            // std::cout<< "Line:" << t.lineNumber() << std::endl;
            // std::cout<< "Column:" << start << std::endl;
            // 
            // t = string(buf);
