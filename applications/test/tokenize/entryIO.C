...
//XXXX
        else if (keyToken.isPunctuation() && keyToken.pToken() == token::DIVIDE)
        {
            DebugVar(keyToken.info());
            token nextToken(is);
            is.putBack(nextToken);
            DebugVar(nextToken.info());
            if
            (
                nextToken.isPunctuation()
             && nextToken.pToken() == token::DIVIDE
            )
            {
                return parentDict.add
                (
                    new functionEntries::commentEntry(keyword, parentDict, is),
                    false       //mergeEntry
                );
            }
            else
