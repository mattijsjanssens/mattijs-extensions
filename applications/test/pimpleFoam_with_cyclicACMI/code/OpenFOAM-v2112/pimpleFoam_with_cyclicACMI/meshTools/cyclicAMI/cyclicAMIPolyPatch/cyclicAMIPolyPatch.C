...
)
{
    if (transform() != neighbPatch(0).transform())
    {
        FatalErrorInFunction
            ...
            << ", neighbour patch " << neighbPatchNames()[0]
            << " has transform type "
...
void Foam::cyclicAMIPolyPatch::resetAMI(const UList<point>& points) const
{
    ...
    forAll(nbrIds, nbri)
    {
        ...
        if (createAMIFaces_)
        {
            // AMI is created based on the original patch faces (non-extended
            // patch)
            if (srcFaceIDs_.size())
            {
        ...
        AMIPtrs_.set(nbri, AMIPtr_->clone());
        AMIPtrs_[nbri].upToDate() = false;
        AMIPtrs_[nbri].calculate(patch0, nbrPatch0, surfPtr());
        ...
        else
        {
            if (debug)
            {
                ...
                AMIPtrs_[nbri].checkSymmetricWeights(true);
            }
        }
    }
...
:
    ...
    (
        ...
        (
            ...
            (
                1,
                dict.lookupOrDefault<word>("neighbourPatch", word::null)
            )
        )
    ),
    coupleGroup_(dict),
...
{
    const labelListList& addSourceFaces = AMI(0)().srcAddress();
    ...
    forAll (addSourceFaces, faceI)
    {
        ...
        forAll (nbrFaceIs, j)
        {
            ...
            if (nbrFaceI < neighbPatch(0).size())
            {
                // local faces
                newFaces++;
            }
...
Foam::refPtr<Foam::AMIPatchToPatchInterpolation>
...
bool Foam::cyclicAMIPolyPatch::applyLowWeightCorrection() const
{
    for (const label i : AMIIndices())
    {
        ...
        if (myAMI.valid())
        {
            if (myAMI().applyLowWeightCorrection())
            {
                return true;
            }
        }
        else
        {
            ...
            if (nbrAMI().applyLowWeightCorrection())
            {
                return true;
            }
        }
