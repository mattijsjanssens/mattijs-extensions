    ...
    )
    {
        if (debug)
        {
            Pout<< "cyclicACMIPolyPatch::updateAreas() :"
                << " patch:" << this->name()
                << " neighbPatches:" << this->neighbPatchNames()
        ...
        scaledMaskSum_ =
            ...
            (
                scalar(1) - tolerance_,
                max(tolerance_, scalePtr_->value(t)*maskSum_)
            );


//        if (!tgtScalePtr_.valid())
//        {
//            tgtScalePtr_= scalePtr_.clone(neighbPatch());
//        }
//
//        tgtScaledMask_ =
//            min
...
//            );
        ...
        if (debug)
        {
            Pout<< "cyclicACMIPolyPatch::updateAreas : scaling masks"
                << " for " << name() << " mask " << gAverage(scaledMaskSum_)
                << " and " << nonOverlapPatch().name()
                << " mask " << gAverage(scaledMaskSum_) << endl;
        }

        // Calculate areas from the masks
        cyclicACMIPolyPatch& cpp = const_cast<cyclicACMIPolyPatch&>(*this);
        cpp.scalePatchFaceAreas(*this, scaledMaskSum_, thisSf_, thisNoSf_);
...
void Foam::cyclicACMIPolyPatch::resetAMI(const UList<point>& points) const
{
    ...
    for (const label nbri : AMIIndices())
    {
        ...
        if (myAMI.valid())
        {
            reportCoverage("source", myAMI().srcWeightsSum());
            reportCoverage("target", myAMI().tgtWeightsSum());
        }
    }
    ...
    if (debug)
    {
        ...
        for (const label nbri : AMIIndices())
        {
            const cyclicACMIPolyPatch& patch = this->neighbPatch(nbri);
            Pout<< "patch:" << patch.name() << " size:" << patch.size()
                << " non-overlap patch: " << patch.nonOverlapPatch().name()
                << " size:" << patch.nonOverlapPatch().size()
...
void Foam::cyclicACMIPolyPatch::scalePatchFaceAreas()
{
    ...
    if (scalePtr_.valid())
    {
        ...
        //nbrSf_.setSize(AMIIndices().size());
        //nbrNoSf_.setSize(AMIIndices().size());
        //for (const label nbri : AMIIndices())
        //{
        //    const cyclicACMIPolyPatch& nbrPatch = this->neighbPatch(nbri);
        //    nbrSf_[nbri] = nbrPatch.faceAreas();
        //    const polyPatch& nbrNonOverlapPatch = nbrPatch.nonOverlapPatch();
        //    nbrNoSf_[nbri] = nbrNonOverlapPatch.faceAreas();
        //}
    }
...
{
    // TBD - multiple AMI
    const List<labelList>& addSourceFaces = AMI(0)().srcAddress();
    const scalarField& fMask = maskSum_;    //srcMask();
...
{
    const scalarField& fMask = maskSum_;
    const labelListList& srcFaces = AMI(0)().srcAddress();
...
const Foam::cyclicACMIPolyPatch&
