...
Foam::cyclicAMIFvPatchField<Type>::patchNeighbourField() const
{
    ...
    const auto& cpp = cyclicAMIPatch_.cyclicAMIPatch();

    tmp<Field<Type>> tpnf;
    if (cpp.applyLowWeightCorrection())
    {
        const Field<Type> pnfInternal(iField, cpp.faceCells());
        tpnf = cpp.interpolate
        (
...
Foam::cyclicAMIFvPatchField<Type>::neighbourPatchField(const label index) const
{
    ...
    (
        fld.boundaryField()[cyclicAMIPatch_.neighbPatchIDs()[index]]
    );
}
...
) const
{
    ...
    solveScalarField pnf(cyclicAMIPatch_.neighbSize());
    {
        label n = 0;
        for (const label index : cyclicAMIPatch_.AMIIndices())
        {
            const label nbrPatchID = cyclicAMIPatch_.neighbPatchIDs()[index];
...
) const
{
    ...
    Field<Type> pnf(cyclicAMIPatch_.neighbSize());
    {
        label n = 0;
        for (const label index : cyclicAMIPatch_.AMIIndices())
        {
...
{
    if (this->cyclicAMIPatch().owner(0))
    {
        ...
        if (matrix.psi(mat).mesh().fluxRequired(this->internalField().name()))
        {
            ...
            const label nbrPathID =
                cyclicAMIPatch_.cyclicAMIPatch().neighbPatchIDs()[0];
...
) const
{
    ...
    const auto& assembly = matrix.lduMeshAssembly();

    const label nSubFaces(assembly.cellBoundMap()[mat][index].size());

    Field<scalar> mapCoeffs(nSubFaces, Zero);

    const scalarListList& srcWeight =
        cyclicAMIPatch_.AMI(0)().srcWeights();
    ...
    forAll(*this, faceI)
    {
        ...
        for(label i=0; i<w.size(); i++)
        {
            const label localFaceId =
                assembly.facePatchFaceMap()[mat][index][subFaceI];
