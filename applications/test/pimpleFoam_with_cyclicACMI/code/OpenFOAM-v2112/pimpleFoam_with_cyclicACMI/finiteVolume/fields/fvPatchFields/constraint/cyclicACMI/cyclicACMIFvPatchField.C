...
Foam::cyclicACMIFvPatchField<Type>::neighbourPatchField(const label index) const
{
    ...
    (
        fld.boundaryField()[cyclicACMIPatch_.neighbPatchIDs()[index]]
    );
}
...
//    solveScalarField pnf(psiInternal, nbrFaceCellsCoupled);
    ...
    //const cyclicACMIPolyPatch& cpp = cyclicACMIPatch_.cyclicACMIPatch();
    solveScalarField pnf(cyclicACMIPatch_.neighbSize());
    {
        label n = 0;
        for (const label index : cyclicACMIPatch_.AMIIndices())
        {
            const label nbrPatchID = cyclicACMIPatch_.neighbPatchIDs()[index];
...
) const
{
    ...
    Field<Type> pnf(cyclicACMIPatch_.neighbSize());
    {
        label n = 0;
        for (const label index : cyclicACMIPatch_.AMIIndices())
        {
...
{
    // TBD. multi-patch
    if (this->cyclicACMIPatch().owner(0))
    {
        ...
        if (matrix.psi(mat).mesh().fluxRequired(this->internalField().name()))
        {
            ...
            const label nbrPathID =
                cyclicACMIPatch_.cyclicACMIPatch().neighbPatchIDs()[0];
...
) const
{
    ...
    const scalarListList& srcWeight =
        cyclicACMIPatch_.cyclicACMIPatch().AMI(0)().srcWeights();
