...
void* Foam::functionEntries::codeStream::findLibrary
(
...
bool Foam::functionEntries::codeStream::writeCode
(
    const dictionary& parentDict,
    dynamicCode& dynCode,
...
void Foam::functionEntries::codeStream::writeCode
(
    const dictionary& parentDict,
...
)
{
    ...
    // see if library is loaded
    void* lib = findLibrary(parentDict, libPath);
    ...
    if (!lib)
    {
        ...
        if (create)
        {
            writeCode(parentDict, dynCode, context);
        }
    }
}
