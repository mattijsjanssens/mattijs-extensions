...
template<class Type>
void Foam::unallocatedGenericFvsPatchField<Type>::read
(
...
//        << "Trying to construct an unallocatedGenericFvsPatchField on patch "
//        << this->patch().name()
...
Foam::unallocatedGenericFvsPatchField<Type>::entryTypes() const
{
    label n =
        scalarFields_.size()
      + vectorFields_.size()
...
template<class Type>
void Foam::unallocatedGenericFvsPatchField<Type>::addEntry
(
    const word& key,
    const word& type
)
