...
{
    string::size_type dotPos = keyword.find(scopeChar);
    ...
    else
    {
        if (dotPos == 0)
        {
            ...
            while (iter != keyword.end() && *iter == scopeChar)
            {
                ++iter;
                ++endVar;
        ...
        else
        {
            ...
            if (!entPtr)
            {
                ...
                while (true)
                {
                    ...
                    nextDotPos = keyword.find(scopeChar, nextDotPos+1);
                }
            }
...
)
{
    ...
    const entry* ePtr = lookupScopedEntryPtr(varName, true, true, scopeChar);
