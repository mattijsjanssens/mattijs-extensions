...
{
    string::size_type i = scopedName.find_last_of(scopeChar);
    if (i != string::npos)
    {
        return Pair<word>
        (
...
int main(int argc, char *argv[])
{
    ...
    (
        "scope",
        "scope character",
        "Override the scope character (default '.')"
    );
    ...
    char scopeChar = '.';
    args.optionReadIfPresent("scope", scopeChar);
    ...
    {
        remove(dict, diffDict, scopeChar);
        dict.write(Info, false);
    }
    else
    {
