...
//! \cond fileScope
//  Find but exclude escaped characters
//
static inline std::string::size_type findNonEscaped
(
    const std::string& s,
    const char& c,
...
{
    std::string::size_type pos = s.find(c, startPos);
    while (pos > 0 && s[pos-1] == '\\')
    {
        pos = s.find(c, pos+1);
    }
...
{
std::cout<< FUNCTION_NAME << "s:" << s << std::endl;
...
//        (begVar = s.find(sigil, begVar)) != string::npos
        (begVar = findNonEscaped(s, sigil, begVar)) != string::npos
     && begVar < s.size()-1
    )
    {
        if (begVar == 0 || s[begVar-1] != '\\')
        {
            ...
            if (s[begVar+1] == '{')
            {
                ...
                endVar = findNonEscaped(s, '}', begVar);
                delim = 1;
...
//            if ((begVar = s.find('/')) != string::npos)
            if ((begVar = findNonEscaped(s, '/', 0)) != string::npos)
            {
...
//        (begVar = s.find('$', begVar)) != string::npos
        (begVar = findNonEscaped(s, '$', begVar)) != string::npos
     && begVar < s.size()-1
    )
    {
        if (begVar == 0 || s[begVar-1] != '\\')
