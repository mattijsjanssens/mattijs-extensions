...
) const
{
    ...
    while (dotPos > 0 && keyword[dotPos-1] == '\\')
    {
DebugVar(keyword);
DebugVar(dotPos);
...
DebugVar(firstWord);
            ...
            if (!entPtr)
            {
                ...
                string::size_type nextDotPos;

                while (true)
                {
                    nextDotPos = keyword.find('.', nextDotPos+1);
                    while (nextDotPos > 0 && keyword[nextDotPos-1] == '\\')
                    {
DebugVar(keyword);
DebugVar(nextDotPos);
                        nextDotPos = keyword.find('.', nextDotPos+1);
...
{
    string::size_type pos = keyword.find("\\.");
    if (pos != string::npos)
    {
        return lookupEntryPtr
        (
